...
# DataName = "Moving MNIST"
DataName = "dMRI_ODF"

if DataName == "dMRI_ODF":
    ...
    for i in range(Track_num):
        if Data is None:
            Data = np.load("./data/ad_data/processed_data/"+label_name+"/odfTrack"+str(Trackids[i])+".npy")
        else:
            tempdata = np.load("./data/ad_data/processed_data/"+label_name+"/odfTrack"+str(Trackids[i])+".npy")
            Data = np.concatenate((Data,tempdata),axis = 1 )
    Data[Data<0.]=0.
    sumData = np.sum(Data,axis = 2, keepdims = True)
    Data = Data/sumData

    class1_num = NagPos[0].size
    class2_num = PosPos[0].size
    batch_size = min(class1_num,class2_num,32)
    # pdb.set_trace()
    ...
    # y = tf.placeholder(np.float32,shape = (None,class_num)) 
    ...
    loss = tf.reduce_sum(tf.pow(pred_last_voxel-Y_pred_voxel,2))
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for epoch in range(pre_train_epoch):
            samp = random.sample(range(class1_num+class2_num),batch_size)
            _, loss_ = sess.run([train_step,loss],
                                        feed_dict={
                                                   X:Data[samp,:-1,:,:],
                                                   Y_pred_voxel:Data[samp,-1:,:,:],
                                                   keep_prob:0.75
                                                    })
        ...
        saver.save(sess, "./data/ad_data/processed_data/"+label_name+"/model/ODFmodel"+str(Trackids[0])+".ckpt")
        ##########
        dW0_ = []
        for k__ in range(5):
            saver.restore(sess, "./data/ad_data/processed_data/"+label_name+"/model/ODFmodel"+str(Trackids[0])+".ckpt")
            for epoch in range(epoch_num):
                samp = random.sample(range(class1_num),batch_size)
                _, loss_ = sess.run([train_step,loss],
                                            feed_dict={
                                                       # X:NagData,
                                                       # Y_pred_voxel:Nag_lastvoxel,
                                                       X:NagData[samp,:,:,:],
                                                       Y_pred_voxel:Nag_lastvoxel[samp,:,:,:],
                                                       keep_prob:0.75
                                                        })
            Weights_Mani_Nag,W_FM_Nag = sess.run([Weights_Mani,W_FM])
            saver.restore(sess, "./data/ad_data/processed_data/"+label_name+"/model/ODFmodel"+str(Trackids[0])+".ckpt")
            for epoch in range(epoch_num):
                samp = random.sample(range(class2_num),batch_size)
                _, loss_ = sess.run([train_step,loss],
                                            feed_dict={
                                                       X:PosData[samp,:,:,:],
                                                       Y_pred_voxel:Pos_lastvoxel[samp,:,:,:],
                                                       keep_prob:0.75
                                                        })
        ...
        for rep in range(group_test_times):
            ...
            for epoch in range(epoch_num):
                ...
                _, loss_ = sess.run([train_step,loss],
                                            feed_dict={
                                                       X:FakeNagData[samp,:,:,:],
                                                       Y_pred_voxel:FakeNag_lastvoxel[samp,:,:,:],
                                                       keep_prob:0.75
                                                        })
            Weights_Mani_Nag,W_FM_Nag = sess.run([Weights_Mani,W_FM])
            ...
            for epoch in range(epoch_num):
                ...
                _, loss_ = sess.run([train_step,loss],
                                            feed_dict={
                                                       X:FakePosData[samp,:,:,:],
                                                       Y_pred_voxel:FakePos_lastvoxel[samp,:,:,:],
                                                       keep_prob:0.75
                                                        })
            Weights_Mani_Pos,W_FM_Pos = sess.run([Weights_Mani,W_FM])
        ...
        print ("The rate of random is smaller than the original is : " , count/group_test_times)
        np.savez("./data/ad_data/processed_data/"+label_name+"/ODFResult"+str(Trackids[0])+".npz",rate = np.asarray(count/group_test_times),In_channel = np.asarray(in_channels),Middle_channel = np.asarray(middle_channels), Out_channel = np.asarray(out_channels),K = np.asarray(k))
